  ...
  return (
    <div className="relative flex min-h-[844px] w-full max-w-[390px] flex-col bg-background-dark overflow-hidden border border-slate-800 shadow-2xl mx-auto sm:my-8 sm:rounded-[2.5rem]">
      ...
      <div className="relative z-10 flex-1 flex flex-col overflow-y-auto">
        ...
        {/* Player's role card (when playing) */}
        {isPlaying && !isHost && (
                ...
                <div className="flex items-center gap-4">
                  <span
                    className={`material-symbols-outlined text-[48px] ${
                      ROLE_COLORS[currentPlayer.role ?? "civilian"]
                    }`}
                  >
                  ...
                  <div className="text-left">
                    <p
                      className={`font-typewriter text-2xl font-bold uppercase tracking-wider ${
                        ROLE_COLORS[currentPlayer.role ?? "civilian"]
                      }`}
                    >
        ...
        {/* Host role overview (when playing) */}
        {isPlaying && isHost && (
          <div className="mx-5 mt-5 p-4 rounded-xl bg-black/40 border border-slate-700">
            ...
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[28px] text-primary">
                {game.phase === "night" ? "bedtime" : game.phase === "day" ? "wb_sunny" : "how_to_vote"}
              </span>
              <span className="font-typewriter text-xl font-bold text-white uppercase tracking-wider">
                {PHASE_LABELS[game.phase]}
              </span>
            </div>
          </div>
              ...
              >
                ...
                >
                  <span className="material-symbols-outlined text-[18px] text-slate-400">
                    {p.isHost ? "manage_accounts" : "person"}
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {p.isYou && (
                      <span className="text-xs text-primary font-typewriter">
                        (ty)
                      </span>
                    )}
                    {p.isHost && (
                      <span className="text-xs text-primary/70 font-typewriter uppercase">
                        MG
                      </span>
                    )}
                  </div>
                  {!p.isAlive && (
                    <span className="text-xs text-slate-600 font-typewriter uppercase">
                      Wyeliminowany
                    </span>
                  )}
                </div>
                {/* Role — visible to host or own role when playing */}
        ...
        {/* Start button (host, lobby, >= 4 players) */}
        {isHost && isLobby && (
        ...
        {error && (
          <div className="mx-5 mb-4">
            <p className="text-primary text-sm font-typewriter text-center">
              {error}
            </p>
          </div>
        )}
